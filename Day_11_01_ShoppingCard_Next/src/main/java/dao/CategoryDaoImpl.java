...
public class CategoryDaoImpl implements CategoryDao {
	...
	public String deleteCategory(String categoryName) {
		...
		try {
			...
			Category catgeory = session.createQuery(jpql, Category.class).setParameter("nm", categoryName)
					.getSingleResult();
			// category : PERSISTENT
			// => no exc => category found !
			session.delete(catgeory); // category : REMOVED (simply marking the entity for removal)
			tx.commit(); // hib perform dirty chking --> session.flush --> delete query , session.close
							// --> L1 cache destroyed , cn rets to the pool
			// category : TRANSIENT
	...
	//	String jpql = "select c from Category c where c.categoryName=:nm";
		...
		try {
			category = session.createQuery(jpql, Category.class).
					setParameter("nm", categoryName).getSingleResult();
			//category : persistent
			//simply provide a hint : fetch size of the collection.
